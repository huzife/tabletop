...
| 实时传输 | 原生 WebSocket，服务端基于 `ws` | 协议完全可控，首期不需要 Socket.IO 附加语义 |
| 数据校验 | Zod | 设置、HTTP、WebSocket 和插件边界共享运行时校验 |
| 数据库 | SQLite WAL + Drizzle | 数据量小、备份简单、迁移可版本化 |
| 密码 | Argon2id | 现代密码哈希，支持可调内存与时间成本 |
| 测试 | Vitest + Playwright | 规则单测、服务集成测试和浏览器流程覆盖 |
| 反向代理 | Nginx | 静态资源、HTTP 与 WebSocket 统一入口 |
...
2. WebSocket 网关只解析通用信封、认证会话和路由房间命令，不理解具体游戏动作载荷。
...
│   └── server/              # Fastify、WebSocket、房间运行时和后台
├── packages/
│   ├── protocol/            # HTTP/WS 信封、错误码和共享 DTO
...
`apps/server/src/rooms/gateway.ts` 在升级握手时验证会话 Cookie，为连接分配 `connectionId`，并把连接绑定到 `accountId` 与登录 `sessionId`。它负责：
...
网关不执行游戏规则，也不根据客户端声称的座位决定权限。座位控制权始终从房间成员状态查询。

### 6.3 房间目录与运行时

`apps/server/src/rooms/registry.ts` 中的 `RoomRegistry` 是进程内房间索引，按 `roomId` 保存 `apps/server/src/rooms/room-runtime.ts` 中的 `RoomRuntime`。每个运行时包含大厅状态、成员、控制连接、最多 100 条的聊天 FIFO 队列、游戏设置、游戏实例、修订号和截止任务。

每个房间拥有逻辑串行命令队列。WebSocket 成员命令、游戏动作、连接变化、截止任务和 AI 返回结果都进入该队列，同一时刻只允许一个此类命令改变房间状态。全站/单游戏关闭和进程停止调用幂等 `destroy()`，属于直接终止房间的例外，不再执行规则转换。Node.js 单线程并不能自动避免异步交错，因此除终止例外外的房间可变状态入口都必须串行化。
...
插件不能直接访问 WebSocket、SQLite、文件系统或全局房间表。必要能力通过窄上下文注入，保证规则可测试且未来可以迁移到其他运行方式。
...
公共外壳位于 `apps/web/src`，提供登录、首页、游戏目录、房间列表、创建房间、房间成员、准备、房主操作、观众列表、聊天、连接提示和管理后台。HTTP 访问集中在 `apps/web/src/api/client.ts`，房间连接集中在 `apps/web/src/rooms/use-room-socket.ts`。公共层不知道游戏棋盘内部状态，只保存协议定义的通用 `RoomSnapshotPayload`。
...
| `connectionId` | 一条 WebSocket 生命周期 | 路由消息与心跳 |
| `roomId` / `matchId` | 房间/单局生命周期 | 防止旧动作进入新局 |

每个 `sessionId` 最多绑定一个活动房间成员身份。同一账号的其他会话可以进入其他房间；如果进入该账号已经占座的同一房间，只能成为观众，不能取得第二个控制连接。

### 9.2 断线、临时控制器与重连

网络断开时，房间队列先把连接标记为不可用，并向游戏宿主提交 `connection.lost` 系统事件。插件可以请求启用兜底控制器，也可以保持座位等待；游戏计时、自动动作和其他状态变化均由插件转换结果决定。平台同时创建 30 秒重连截止任务，把原 `sessionId` 作为恢复凭据。
...
窗口到期时，平台把成员标记为 `offline`、释放该会话的活动房间绑定，并向插件提交 `connection.grace_expired`。插件可以返回结束比赛、继续自动控制、释放座位、保留取回权或其他 SDK 支持的通用指令；房间核心只校验和执行指令，不根据 `gameId` 推断结果。离线房主当时没有在线候选时暂留身份，后续有成员连接或恢复时再次执行通用房主转移。

窗口到期后的通用成员规则如下：

- 无座观众直接从房间成员表清除；若因此没有任何真人成员，房间销毁。
- 仍有关联座位的离线成员可以按插件指令保留。原 `sessionId` 后续重新取得 join ticket 时复用该成员，而不是创建第二个成员或第二份座位归属。
- 离线成员若是房主，平台优先把房主转移给最早加入且当前已连接的其他真人成员；当没有其他在线成员时不虚构房主身份，原成员仍保留为房主，直到后续成员变化触发合法转移。

同一账号的新会话不等于原会话恢复，只能按普通加入和插件开放的 `room.seat.reclaim` 权限行动。每款游戏对比赛结果、控制器和取回权的选择只能在对应游戏文档和插件实现中定义。
...
- HTTP 和 WebSocket 都从服务端会话解析账号，不接受客户端传入 `accountId`。
...
| 自定义 WebSocket 协议 | 语义和版本完全可控 | 需要自行实现重连与幂等 |
