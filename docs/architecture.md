...
`apps/server/src/rooms/gateway.ts` 为 WebSocket 和 HTTP 长轮询分配统一的 `connectionId`，并把连接绑定到服务端解析的 `accountId` 与登录 `sessionId`。每条连接最多再绑定一个房间，所有会话共享账号级唯一当前房间。WebSocket 在升级握手时验证 Origin 和会话；长轮询的创建、poll、命令与关闭接口验证会话、同源 Origin 和 CSRF。网关负责：
...
`apps/server/src/rooms/registry.ts` 中的 `RoomRegistry` 是进程内房间索引，按 `roomId` 保存 `apps/server/src/rooms/room-runtime.ts` 中的 `RoomRuntime`，并按 `accountId` 保存至多一个当前房间和成员绑定。查询会同时校验房间成员并修复丢失或陈旧的辅助索引。每个运行时包含大厅状态、成员、控制连接、最多 100 条的聊天 FIFO 队列、游戏设置、游戏实例、修订号和截止任务。
...
每条连接最多绑定一个房间；账号级索引只保存空或一个 `{ roomId, memberId }`，因此同一账号的不同登录会话、标签页和传输不能同时加入不同房间。成员只公开 `connected`（正常）和 `reconnecting`（重连）两种连接状态。新连接若目标是当前房间，可以接管旧连接；若目标是其他房间，则在创建、签发 ticket 和消费 ticket 三个边界上拒绝。

### 9.2 断线、临时控制器与重连

非对局阶段网络断开或页面离开时，房间队列立即移除成员和座位、清空账号当前房间并广播新快照，不创建恢复窗口。房间页面通过 `pagehide` 的 keepalive 关闭请求和 SPA 卸载时的传输关闭主动通知服务端，心跳与长轮询租约负责无法主动通知的异常情况。

对局阶段的网络断开、页面回退、刷新或关闭会把成员改为 `reconnecting`，向游戏宿主提交 `connection.lost` 并创建完整的 30 秒截止任务。插件可以请求启用兜底控制器，也可以保持座位等待；游戏计时、自动动作和其他状态变化均由插件转换结果决定。账号当前房间在窗口内保持非空，首页通过 `currentRoomId` 提供返回入口。

同一账号的任一有效登录会话在窗口内返回时，平台取消截止任务，向插件提交 `connection.restored`，并向浏览器发送最新完整投影快照。若浏览器已确认旧链路失效、服务端网络栈却仍把它视为在线，新连接会直接替换旧 `connectionId`；旧连接随后到达的关闭事件因 ID 不匹配而被忽略。插件已经接受的自动动作不会因网络恢复而回滚。下图展示主动退出、可选临时接管、快照恢复和窗口到期路径。

![图9-1 断线、可选临时接管与重连](images/architecture-fig03.png)

用户点击“离开房间”且确认后不进入重连流程：平台立即提交 `member.left`，删除成员并清空账号当前房间。被动断开的窗口到期时，平台先提交 `connection.grace_expired`，插件可以返回结束比赛、继续自动控制或释放座位等通用指令；房间核心随后删除成员、清空账号当前房间并清除取回权限，不根据 `gameId` 推断比赛结果。退出成员是房主时转给最早加入的剩余真人成员。

窗口到期后的通用成员规则如下：

- 无论是否关联座位，主动退出或到期成员都从房间成员表清除，账号之后普通加入会创建新的观战成员。
...
同一账号的新会话可以返回账号当前房间并接管连接，但不能创建第二个成员或加入其他房间。每款游戏对比赛结果和退出后的控制器选择只能在对应游戏文档和插件实现中定义。
