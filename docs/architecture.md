...
窗口到期时，平台把成员标记为 `offline`、只释放对应的 `sessionId + roomId` 绑定，并向插件提交 `connection.grace_expired`。插件可以返回结束比赛、继续自动控制、释放座位、保留取回权或其他 SDK 支持的通用指令；房间核心只校验和执行指令，不根据 `gameId` 推断结果。同一会话在其他房间的绑定和连接不受影响。离线房主当时没有在线候选时暂留身份，后续有成员连接或恢复时再次执行通用房主转移。

窗口到期后的通用成员规则如下：

- 无座观众直接从房间成员表清除。
- 仍有关联座位的离线成员可以按插件指令保留。原 `sessionId` 后续重新取得同一房间的 join ticket 时复用该成员，而不是创建第二个成员或第二份座位归属。
- 主动离开或任一重连窗口到期后，平台重新检查房间成员；除空座单人练习房特例外，只要仍有 `connected` 或 `reconnecting` 成员就保留房间，否则立即销毁。因而最后一条连接关闭后不会立即销毁其他房间，所有断线成员都能获得完整 30 秒恢复窗口。
