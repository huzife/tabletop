...
`apps/server/src/rooms/gateway.ts` 为 WebSocket 和 HTTP 长轮询分配统一的 `connectionId`，并把连接绑定到服务端解析的 `accountId` 与登录 `sessionId`。WebSocket 在升级握手时验证 Origin 和会话；长轮询的创建、poll、命令与关闭接口验证会话、同源 Origin 和 CSRF。网关负责：
