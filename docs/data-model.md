...
“设备”在首期等价于一个登录会话。浏览器退出再登录会创建新 `sessionId`；一个会话可以通过不同连接同时进入多个不同房间，进程内连接目录按 `sessionId + roomId` 保存各房间成员绑定，而每个 `connectionId` 仍最多绑定一个房间。同一账号可以在同一房间拥有多个观战成员身份，但房间运行时保证该账号最多占据一个真人座位。这些关系都不写入 SQLite。
