...
“设备”在首期等价于一个登录会话。浏览器退出再登录会创建新 `sessionId`，但所有会话共享账号级当前房间。进程内房间目录按 `accountId` 保存空或一个 `{ roomId, memberId }`，每个 `connectionId` 仍最多绑定一个房间；房间成员连接状态只有 `connected` 和 `reconnecting`。创建、加入和恢复都以账号当前房间为准，这些关系不写入 SQLite，服务器重启时与房间一起消失。
...
在线状态不在数据库中。删除账号、登录以及账号变更使用应用级账号锁；删除服务在锁内检查房间连接目录和房间成员索引，只有账号既不在线也没有残留房间成员时才执行删除事务。
