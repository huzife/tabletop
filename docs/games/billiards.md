...
非对局阶段离开或断开时，平台立即移除成员和座位。对局阶段的手动离开、页面回退/刷新/关闭和网络断开统一进入 30 秒恢复窗口；台球不启用临时控制器，因此恢复期间局面保持不变，大厅向原登录会话提供“重新进入”。窗口内恢复只接收当前权威快照，不回放旧动画。

双人对局恢复窗口到期后，断线方以 `disconnected` 结束本局，对手获胜，平台删除断线成员和座位。单人练习窗口到期后释放练习座位并销毁空座练习房。台球不支持到期后的手动座位取回；账号禁用等强制移除在双人对局中以 `left` 结束。
