...
| 击球 | 上游默认输入 `V0` 是 m/s，且默认二维 resolver 不产生竖直速度，也没有滑杆裁决 | 房间使用 `[1,150]` 力度并线性映射到 `V0`；`50 → 2 m/s`，`150 → 6 m/s`。`miscue` 仅为产品诊断，不改变上游公式 |
...
非对局阶段离开或断开时，平台立即移除成员和座位。对局阶段的页面回退、刷新、关闭和网络断开进入 30 秒恢复窗口；台球不启用临时控制器，因此恢复期间局面保持不变，首页向该账号提供返回当前房间入口。窗口内恢复只接收当前权威快照，不回放旧动画。用户确认手动退出后立即以 `left` 结束双人对局，不进入恢复窗口；单人练习则立即释放座位并销毁房间。

双人对局恢复窗口到期后，断线方以 `disconnected` 结束本局，对手获胜，平台删除断线成员和座位。单人练习窗口到期后释放练习座位并销毁空座练习房。台球不支持退出后的手动座位取回；手动退出或账号禁用等强制移除在双人对局中以 `left` 结束。
...
| `power` | `[1, 150]` | 线性映射到 Pooltool 球杆速度 `V0 = power / 25 m/s` |
