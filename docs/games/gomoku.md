...
认输是明确的游戏动作，立即生效并由对方获胜。手动离开房间、页面回退/刷新/关闭和网络断开统一按第 7.4 节进入 30 秒恢复窗口，不直接等价于认输；恢复失败后以断线判负。非对局阶段发生这些离开行为时由平台立即清理成员。服务关闭以 `service_disabled` 终止，不计任何玩家胜负，也不保存战绩。
...
| `member.left`              | 仅用于账号禁用等强制移除，立即以 `endReason=left` 判负                             |

平台的恢复窗口固定为 30 秒，大厅在窗口内为原登录会话显示“重新进入”。恢复只接受原登录会话；若兜底控制器在窗口内已经形成胜负，重连只显示结束局面，不再交回控制权。窗口到期后成员被删除，不能再按普通加入取回原座位。连珠规则虽然不开放房主可添加的 AI 座位，仍使用第 8 节定义的合法兜底控制器处理短暂断线。
...
- 手动离开与网络断开统一触发临时接管、30 秒内恢复、`connection.grace_expired` 判负、成员清理和服务关闭。
