...
认输是明确的游戏动作，立即生效并由对方获胜。页面回退、刷新、关闭和网络断开按第 7.4 节进入 30 秒恢复窗口，恢复失败后以断线判负。对局中点击“离开房间”会先收到不可重连警告；确认后立即退出并以离开判负。非对局阶段发生这些离开行为时由平台立即清理成员。服务关闭以 `service_disabled` 终止，不计任何玩家胜负，也不保存战绩。
...
| `connection.grace_expired` | 断线玩家立即判负，`endReason=disconnected`，本局结束；平台随后移除成员和座位       |
| `member.left`              | 用于用户确认手动退出或账号强制移除，立即以 `endReason=left` 判负                   |

平台的恢复窗口固定为 30 秒，首页在窗口内通过账号当前房间提供返回入口；同一账号的任一有效会话都可恢复。若兜底控制器在窗口内已经形成胜负，重连只显示结束局面，不再交回控制权。窗口到期或主动退出后成员被删除，不能再按普通加入取回原座位。连珠规则虽然不开放房主可添加的 AI 座位，仍使用第 8 节定义的合法兜底控制器处理短暂断线。
...
- 手动退出确认后立即以 `member.left` 判负；被动断开触发临时接管、30 秒内恢复、`connection.grace_expired` 判负、成员清理和服务关闭。
