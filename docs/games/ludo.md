...
| `member.left`              | 仅用于账号禁用等强制移除，立即由持续 AI 接管；若已无任何在线或待恢复成员，房间按平台规则销毁                       |

手动离开房间、页面回退/刷新/关闭和网络断开在对局中统一进入 30 秒窗口，大厅为原会话显示“重新进入”。窗口内恢复会交还真人控制，但 AI 已完成的投骰、移动、碰撞、排名和其他状态变化均不回滚。窗口到期后视为退出，平台删除成员和恢复绑定，原账号以后即使重新加入也只能观战，不能取回该局座位。
...
- AI 合法性、手动离开与断线的临时接管、窗口内恢复、窗口到期成员清理、不可取回的持续接管和完成排名。
