...
| `connection.grace_expired` | 临时 AI 转为持续 AI并返回 `seat.setReclaimable(false)`；平台随后删除成员但让 AI 完成本局                            |
| `seat.reclaim_requested`   | 由原账号的 `room.seat.reclaim` 命令触发；校验通过后返回 `seat.returnHumanControl` 和 `seat.setReclaimable(false)` |
| `member.left`              | 用于用户确认手动退出或账号强制移除，立即由持续 AI 接管；若已无任何在线或待恢复成员，房间按平台规则销毁             |

页面回退、刷新、关闭和网络断开在对局中进入 30 秒窗口，首页通过账号当前房间提供返回入口。窗口内恢复会交还真人控制，但 AI 已完成的投骰、移动、碰撞、排名和其他状态变化均不回滚。用户确认手动退出后由持续 AI 立即接管；窗口到期也视为退出。两种退出都会删除成员、清空账号当前房间，原账号以后即使重新加入也只能观战，不能取回该局座位。
...
- AI 合法性、手动退出的立即持续接管、被动断线的临时接管、窗口内恢复、窗口到期成员清理、不可取回的持续接管和完成排名。
