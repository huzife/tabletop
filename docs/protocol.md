...
- 对局中的刷新或重连不补发所有历史动作，直接用最新快照恢复；非对局阶段断开会先移除旧成员，再按普通加入流程重新进入。
...
页面触发 `pagehide` 时，浏览器还会对当前 `connectionId` 发送带 `keepalive` 的连接关闭请求；SPA 页面回退或组件卸载会关闭 WebSocket 或长轮询。这条主动通知用于尽快同步刷新、关闭标签页和页面跳转，服务端心跳与长轮询租约仍负责通知未送达、浏览器崩溃和物理断网等情况。
