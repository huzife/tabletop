...
系统包含唯一管理员、普通注册用户、房间玩家、观众和 AI 五类角色。普通用户在当前房间中担任玩家或观众；AI 只占据游戏座位，不拥有账号和登录会话。
...
| AUTH-08 | 同一账号允许多设备登录，但账号在任意时刻最多属于一个房间；所有会话和连接共享账号级“当前房间” |
| AUTH-09 | 账号的当前房间只有空和一个 `roomId` 两种状态；房间内该账号只有 `connected`（正常）或 `reconnecting`（重连）两种成员连接状态，每条连接仍最多绑定一个房间 |
...
| ROOM-12 | 非对局阶段房主离开或断开时立即移除并把房主转给最早进入的剩余真人成员；对局阶段房主断开后保留到恢复窗口到期，届时移除并转给最早进入的剩余真人成员 |
| ROOM-13 | 每个房间最多 10 名观众；观众可以查看公开视图和参与房间聊天 |
| ROOM-14 | 对局结束后保留房间、座位和聊天，玩家重新准备后可以再来一局 |
| ROOM-15 | 非对局阶段的手动离开、页面回退/刷新/关闭或连接丢失都立即移除成员、清空账号当前房间并广播新快照；对局阶段只有页面回退/刷新/关闭或连接丢失进入 30 秒恢复窗口。用户点击“离开房间”时必须先确认，确认后立即视为退出、提交 `member.left` 并清理成员，不得再重连 |
| ROOM-16 | 单机练习房固定不出现在公开房间列表，只能由创建者或通过邀请链接进入；声明 `soloPractice` 能力的练习房在全部座位变空时立即销毁，即使仍有观众处于 `connected` 或 `reconnecting` 状态 |
| ROOM-17 | `GET /rooms` 返回账号级可空 `currentRoomId`；首页在非空时提供“返回当前房间”，目标房间入口直接返回，不重复校验密码或申请 join ticket |
| ROOM-18 | 当前房间非空时，创建房间、列表进入和邀请进入其他房间都必须被服务端拒绝；前端在发起已知冲突操作前提醒用户，并询问是否跳转到当前房间 |

单机练习房至少包含一名真人。声明 `bots` 能力的插件由创建者从公开的 `botProfiles` 中选择练习 AI，平台用该 AI 填充其余座位；声明 `soloPractice` 能力的无 AI 插件保留其余座位为空，并允许一个已准备真人开始练习，受邀成员只能观战而不能占用空座。断线玩家在座位释放前仍享有完整 30 秒重连窗口；玩家主动离开或窗口到期并按插件策略释放最后一个座位后，房间立即销毁，观众连接不延长这类空座练习房的生命周期。普通联机房仍遵守 manifest 的 `minPlayers`，单人放宽只适用于同时标记为练习房且由插件明确声明支持的房间。平台在创建前调用插件开局校验，设置或座位组合不兼容时拒绝创建。练习房不出现在公开列表中，只能由创建者或邀请链接进入。
...
| NET-04 | 仅在对局进行中，服务端为被动断开的账号当前房间保留 30 秒重连窗口；同一账号的任一有效会话可恢复并取得当前完整投影快照。旧连接尚未被网络栈判定断开时，新连接可直接接管，旧连接的迟到关闭不得覆盖新绑定 |
| NET-05 | 服务器重启会终止全部房间和对局，不要求从 SQLite 恢复 |
| NET-06 | 对局中的被动断开提交 `connection.lost`，恢复和窗口到期分别提交对应系统事件；确认后的手动退出直接提交 `member.left`。窗口到期或主动退出后平台必须移除成员并禁止后续座位取回，插件只决定比赛结局以及退出后座位是否由自动控制器继续 |
| NET-07 | 平台不得在公共房间或网络模块中根据 `gameId` 分支处理断线结果，也不得回滚临时控制器已经被插件接受的动作 |
| NET-08 | 除以 `connection.pong` 响应的连接心跳和尽力转发且不改变状态的临时游戏事件外，每个成功命令返回带 `stateChanged` 的 `command.ack`；状态变化另外广播按接收者投影的完整快照 |
| NET-09 | 重连窗口到期后清空账号当前房间并从成员表移除用户；插件若让自动控制器继续在局座位，该座位不可再取回。除空座 `soloPractice` 练习房特例外，房间没有 `connected` 或 `reconnecting` 成员时销毁 |
| NET-10 | 首次 join 在传输中断导致结果不明确时，账号已有当前房间但成员从未连接，可用 `room.resume` 完成首次 attach，不向插件伪造恢复事件 |
| NET-11 | 房间连接优先使用 WebSocket；构造、握手或首个快照超时后自动使用同源 HTTP 长轮询，后备传输失败时重新探测 WebSocket，并共享会话鉴权、命令 schema、限流、去重、单连接单房间绑定和快照语义；同一账号的新房间连接只允许返回其当前房间 |
| NET-12 | 插件可声明临时事件 schema，用独立限频、可合并、无修订号和无成功 ACK 的通道同步瞄准等展示数据；服务端必须验证当前行动座位并填写发送座位，临时事件不得改变权威状态或参与规则判断 |
| NET-13 | WebSocket 同时使用协议控制帧和应用层 `connection.ping/pong` 检测半开连接；客户端按带随机抖动的递增间隔重试，长轮询的建连、命令与 poll 都必须有高于服务端正常等待时间的有限超时 |
| NET-14 | 房间页面在 `pagehide` 时使用带 `keepalive` 的连接关闭请求主动通知服务端，SPA 回退或组件卸载同时关闭当前传输；服务端心跳与长轮询租约继续作为浏览器崩溃、断网或通知未送达时的兜底 |

30 秒窗口是对局阶段被动断开的平台连接机制，不是游戏规则。窗口内自动恢复要求账号当前房间与目标房间匹配，不限定原登录会话；窗口到期后平台执行插件的到期转换，随即移除成员并清空账号当前房间。主动退出不进入窗口。非对局阶段即时移除、房主转移、全员不再处于 `connected` 或 `reconnecting` 状态时销毁房间，以及空座 `soloPractice` 练习房不由观众连接保活，都属于平台成员生命周期；比赛结果和退出后的自动控制策略仍由每款游戏在自己的设计文档中定义。
