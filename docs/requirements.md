...
| G-03 | 实时联机 | 少量玩家通过 WebSocket 获得一致、低延迟的权威局面 |
| G-04 | 单人练习 | 游戏可以用服务器 AI 填充空座，无需实现离线网页 |
...
单机练习是一个至少包含一名真人、其余座位由服务器 AI 占据的房间。创建者从插件公开的 `botProfiles` 中选择练习 AI，平台在创建前调用插件开局校验，设置与 AI 不兼容时拒绝创建。此类房间默认仍是公开房间模型，但不出现在公开列表中，只能由创建者或邀请链接进入。
...
| NET-03 | 平台支持插件在 WebSocket 断开后启用临时自动控制器；是否启用以及临时控制器执行什么动作由游戏插件声明 |
...
| AI-03 | AI 计算不得阻塞 WebSocket 事件循环；生产默认使用全站单 Worker Thread 串行执行，并对每个任务设置硬超时 |
...
| WebSocket、修订号、快照和 30 秒重连机制 | 合法动作、状态迁移、随机与计时规则 |
...
2. **端到端骨架**：账号、首页、房间和服务端权威 WebSocket 基础可运行。
