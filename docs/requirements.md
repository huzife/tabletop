...
| ROOM-15 | 除 `ROOM-16` 的空座单人练习房特例外，主动离开或重连窗口到期后，若房间已没有 `connected` 或 `reconnecting` 成员，则立即销毁房间、AI 和聊天记录；`reconnecting` 成员在完整 30 秒窗口内不会触发销毁 |
...
| NET-04 | 服务端按原登录会话与房间保留 30 秒重连窗口；窗口内以 `sessionId + roomId` 恢复后发送当前完整投影快照并取消对应截止任务；旧连接尚未被网络栈判定断开时，同一会话同一房间的新连接可直接接管，旧连接的迟到关闭不得覆盖新绑定 |
...
| NET-09 | 重连窗口到期后清除无座离线观众并只释放该 `sessionId + roomId` 绑定；仍有关联座位的成员可按插件策略保留，原会话再次加入同一房间时复用原成员身份；除空座 `soloPractice` 练习房特例外，房间没有 `connected` 或 `reconnecting` 成员时销毁 |
...
30 秒窗口是平台连接机制，不是游戏规则。窗口内自动恢复要求原账号、原登录会话和目标房间同时匹配；窗口到期后平台只释放该房间的连接恢复凭据，再执行插件返回的通用房间指令。同一会话在其他房间的连接与绑定不受影响。无座观众清理、同会话同房间成员复用、离线房主转移、全员不再处于 `connected` 或 `reconnecting` 状态时销毁房间，以及空座 `soloPractice` 练习房不由观众连接保活，都属于平台成员生命周期；比赛结果、临时接管和后续取回仍由每款游戏在自己的设计文档中定义。
