...
| AUTH-08 | 同一账号允许多设备登录；同一登录会话也可以通过不同房间连接同时进入多个不同房间 |
| AUTH-09 | 每条房间连接最多绑定一个房间；同一账号可以在同一房间拥有多个观战成员身份，但最多占据一个真人座位 |
...
| ROOM-15 | 非对局阶段的手动离开、页面回退/刷新/关闭或连接丢失都立即移除成员、释放绑定并广播新快照；对局阶段的上述行为统一进入 30 秒恢复窗口，窗口到期后视为退出并移除成员。处理后若房间没有 `connected` 或 `reconnecting` 成员，则立即销毁房间、AI 和聊天记录 |
| ROOM-16 | 单机练习房固定不出现在公开房间列表，只能由创建者或通过邀请链接进入；声明 `soloPractice` 能力的练习房在全部座位变空时立即销毁，即使仍有观众处于 `connected` 或 `reconnecting` 状态 |
| ROOM-17 | 房间列表按当前登录会话标记可恢复房间；当前会话仍有房间绑定时显示“重新进入”并直接执行恢复，不重复校验房间密码或申请 join ticket |
...
| NET-04 | 仅在对局进行中，服务端按原登录会话与房间保留 30 秒重连窗口；窗口内以 `sessionId + roomId` 恢复后发送当前完整投影快照并取消对应截止任务；旧连接尚未被网络栈判定断开时，同一会话同一房间的新连接可直接接管，旧连接的迟到关闭不得覆盖新绑定 |
| NET-05 | 服务器重启会终止全部房间和对局，不要求从 SQLite 恢复 |
| NET-06 | 对局中的手动退出与连接丢失统一提交 `connection.lost`；连接恢复和窗口到期继续提交对应系统事件。窗口到期后平台必须移除成员并禁止后续座位取回，插件只决定比赛结局以及退出后座位是否由自动控制器继续 |
...
| NET-09 | 重连窗口到期后释放该 `sessionId + roomId` 绑定并从成员表移除用户；插件若让自动控制器继续在局座位，该座位不可再取回。除空座 `soloPractice` 练习房特例外，房间没有 `connected` 或 `reconnecting` 成员时销毁 |
| NET-10 | 首次 join 在传输中断导致结果不明确时，已预绑定但从未连接的原会话可以用 `room.resume` 完成首次 attach，不向插件伪造恢复事件 |
| NET-11 | 房间连接优先使用 WebSocket；构造、握手或首个快照超时后自动使用同源 HTTP 长轮询，后备传输失败时重新探测 WebSocket，并共享会话鉴权、命令 schema、限流、去重、单连接单房间绑定和快照语义；同一会话面向不同房间的连接互不接管 |
...
30 秒窗口是对局阶段的平台连接机制，不是游戏规则。窗口内自动恢复要求原账号、原登录会话和目标房间同时匹配；窗口到期后平台执行插件的到期转换，随即移除成员和该房间的恢复绑定。同一会话在其他房间的连接与绑定不受影响。非对局阶段即时移除、离线房主转移、全员不再处于 `connected` 或 `reconnecting` 状态时销毁房间，以及空座 `soloPractice` 练习房不由观众连接保活，都属于平台成员生命周期；比赛结果和退出后的自动控制策略仍由每款游戏在自己的设计文档中定义。
